...
namespace MyCashFlowSharp.Infrastructure.Policies
{
    public partial class SmartRetryExecutionPolicy
    {
        private class LeakyBucket
        {
            ...
            {
                //MyCashFlow Plus customers have a bucket that is twice the size (80) so we resize the bucket capacity accordingly
                //It is apparently possible to request the bucket size to be even larger
                //https://ecommerce.MyCashFlow.com/c/MyCashFlow-apis-and-technology/t/what-is-the-default-api-call-limit-on-MyCashFlow-stores-407292
                ...
                //Corrects the grant capacity of the bucket based on the size returned by MyCashFlow.
                //MyCashFlow may know that the remaining capacity is less than we think it is (for example if multiple programs are using that same token)
                //MyCashFlow may also think that the remaining capacity is more than we know, but we do not ever empty the bucket because MyCashFlow is not
