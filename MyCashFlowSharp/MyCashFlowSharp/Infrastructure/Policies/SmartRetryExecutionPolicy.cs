...
{
    /// <summary>
    /// A retry policy that attemps to pro-actively limit the number of requests that will result in a MyCashFlowRateLimitException
    ...
    /// See https://help.MyCashFlow.com/api/guides/api-call-limit
    /// https://en.wikipedia.org/wiki/Leaky_bucket
    /// </remarks>
    public partial class SmartRetryExecutionPolicy : IRequestExecutionPolicy
    {
        private const string RESPONSE_HEADER_API_CALL_LIMIT = "X-MyCashFlow-Shop-Api-Call-Limit";

        private const string REQUEST_HEADER_ACCESS_TOKEN = "X-MyCashFlow-Access-Token";
        ...
        public async Task<T> Run<T>(CloneableRequestMessage baseRequest, ExecuteRequestAsync<T> executeRequestAsync)
        {
            ...
            while (true)
            {
                ...
                catch (MyCashFlowRateLimitException)
                {
                    ...
                    //-MyCashFlow may have a slightly different algorithm
                    //-MyCashFlow may change to a different algorithm in the future
